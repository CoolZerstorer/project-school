...
def open_search_window():
    ...
    student_id_lbl = tk.Label(
        result_frame,
        text="Student ID: --",
        fg="white",
        **lbl_res_opts
    )
    student_id_lbl.pack(fill="x", padx=20, pady=5)

    student_name_lbl = tk.Label(
        result_frame,
        text="Student Name: --",
        fg="white",
        **lbl_res_opts
    )
    student_name_lbl.pack(fill="x", padx=20, pady=5)

    issue_date_lbl = tk.Label(
        result_frame,
        text="Issued Date: --",
        fg="white",
        **lbl_res_opts
    )
    issue_date_lbl.pack(fill="x", padx=20, pady=5)

    status_result_lbl = tk.Label(
        result_frame,
    ...
    def perform_search():
        ...
        if found_id is None:
            id_result_lbl.config(
                text="Book ID: Not Found",
                fg="#f44336"
            )

            name_result_lbl.config(
                text=f"Book Name: '{search_entry.get()}' missing from catalog",
                fg="#f44336"
            )

            student_id_lbl.config(
                text="Student ID: --",
                fg="#f44336"
            )

            student_name_lbl.config(
                text="Student Name: --",
                fg="#f44336"
            )

            issue_date_lbl.config(
                text="Issued Date: --",
                fg="#f44336"
            )

            status_result_lbl.config(
                text="Availability Status: Unknown",
                fg="#f44336"
            )
            return
        ...
        issue_date_lbl.config(
            text=f"Issued Date: {issue_date}",
            fg="white"
        )

        if is_issued:
            status_result_lbl.config(
                text="Availability Status: NOT AVAILABLE (Issued)",
                fg="#ff6b6b"
            )
        else:
            status_result_lbl.config(
                text="Availability Status: AVAILABLE",
                fg="#4caf50"
            )
...
def open_tables_window():
    ...
    def clear_selected_record():
        ...
        # --- NEW PASSWORD PROMPT WINDOW FOR DELETING/CLEARING ---
        pass_prompt = tk.Toplevel(tables_window)
        pass_prompt.title("Authorization Required")
        pass_prompt.geometry("360x180")
        pass_prompt.configure(bg="#222222")
        pass_prompt.resizable(False, False)
        pass_prompt.transient(tables_window) # Keeps window attached on top of records screen
        pass_prompt.grab_set() # Block main window interactions until managed
        
        tk.Label(pass_prompt, text="Enter Security Token to Clear Record:", font=("Segoe UI", 11, "bold"), bg="#222222", fg="#FFFCC6").pack(pady=(20, 10))
        
        pwd_entry = tk.Entry(pass_prompt, show="*", font=("Segoe UI", 12), bg="#333333", fg="white", insertbackground="white", relief="flat", width=22)
        pwd_entry.pack(pady=5)
        pwd_entry.focus()

        def verify_action():
            if pwd_entry.get() == "secret123":
                pass_prompt.destroy() # Close token check block safely
                
                # Execute original code functionality safely down matching schemas
                try:
                    conn = mysql.connector.connect(host="127.0.0.1", user="root", password="root123456", database="student_db")
                    cursor = conn.cursor()
        ...
        # Allow executing verification by hitting the Return key
        pass_prompt.bind('<Return>', lambda event: verify_action())

        confirm_btn = tk.Button(pass_prompt, text="Verify Access", font=("Segoe UI", 10, "bold"), bg="#4CAF50", fg="black", width=14, relief="flat", command=verify_action)
...
def open_update_window():
    ...
    grid_opts = {"row": 0, "column": 2, "padx": 15, "pady": 15}
    fetch_btn.grid(**grid_opts)
...
def open_login_screen():
    ...
    def toggle_password():
        if password_entry.cget("show") == "*":
            ...
            toggle_btn.config(text="Hide", bg="#000000")
        else:
            password_entry.config(show="*")
            toggle_btn.config(text="Show", bg="#000000")

    toggle_btn = tk.Button(password_frame, text="Show", font=("Segoe UI", 9, "bold"), command=toggle_password, width=6, bg="#333333", fg="black", relief="flat")
    ...
    login_button = tk.Button(login_win, text="Authenticate System", font=("Segoe UI", 12, "bold"), command=check_login, bg="#007AFF", fg="black", relief="flat", width=20)
...
title_label = tk.Label(root, text="HOME", font=("Segoe UI", 60, "bold"), bg="#121212", fg="#E1E1E1")
...
invbtn = ttk.Button(root, text="Inventory", style="Custom.TButton", width=15, command=open_inventory_window)
...
invbtn.pack(side="top", anchor="w", padx=(100, 0), pady=(0, 45))
exit_btn.pack(side="top", anchor="w", padx=(100, 0), pady=(0, 45))

vertical_line = tk.Frame(root, width=6, bg='#007AFF') 
vertical_line.place(relx=0.5, rely=0.18, relheight=0.9, anchor="n")
...
try:
    ...
    image_label.place(relx=0.71, rely=0.021, anchor="ne")
