...
def open_tables_window():
    ...
    tables_window.geometry("950x550") 
    ...
    columns = ("book_id", "student_name", "date_of_issue", "book_name", "fine")
    ...
    def load_table_records():
        ...
        try:
            ...
            for row in rows:
                b_id, s_name, issue_date, b_name = row
                
                # Dynamic datetime calculation logic
                if isinstance(issue_date, str):
                    # Fallback context in case string formats leaked inside the DB
                    issue_date_obj = datetime.strptime(issue_date, "%Y-%m-%d").date()
                else:
                    issue_date_obj = issue_date

                days_passed = (today - issue_date_obj).days
                
                if days_passed > 15:
                    fine_amt = (days_passed - 15) * 100
                    fine_str = f"₹{fine_amt} ({days_passed - 15} days overdue)"
                else:
                    fine_str = "₹0 (Safe)"
                
                # FIXED: Structural 5-value array matching schema configuration mapping properties
                tree.insert("", tk.END, values=(b_id, s_name, issue_date_obj.strftime('%Y-%m-%d'), b_name, fine_str))
        except Error as e:
    ...
    def clear_selected_record():
        ...
        try:
            ...
            messagebox.showinfo("Returned Successfully", "Record wiped clean. Book is available in inventory once again.")
            load_table_records() 
        except Error as e:
            messagebox.showerror("Database Execution Error", f"Could not modify entry schema indices: {e}")
        finally:
            if 'conn' in locals() and conn.is_connected():
                cursor.close()
                conn.close()
